...
def func(ev,w_obj):
    ...
    P_A = stage_A.func(fces_ts,fces_cluster,w_obj)
    P_B,SoC = stage_B.func(P_A,fces_cluster,ev)
...
def FCES(ev):
    ...
    # def DataFrame
    time_series_list = ['capacity', 'demand', 'pcs', 'minimumSOC', 'maximumSOC']
    header = []
    dur = 0
    ...
    for arr in range(len(arr_set)):
        ...
        for v in range(int(fces_cluster['n_ev'][arr])):
            ...
            for l in lists:
                ...
                lists = ['minimumSOC', 'maximumSOC']
            for l in lists:
                temp.loc[:ev['duration'][vdx]-1,l] += ev[l][vdx] * ev['capacity'][vdx] /100
            temp.loc[ev['duration'][vdx]-1:,'demand'] += ev['goalSOC'][vdx] * ev['capacity'][vdx] /100
        temp_header = []
        for l in time_series_list:
